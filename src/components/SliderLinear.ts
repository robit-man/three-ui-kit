/**
 * SliderLinear - horizontal slider with line track and ring handle.
 * Header text/readout is laid out separately from track geometry.
 */

import { Mesh, PlaneGeometry, RingGeometry, CircleGeometry } from "three";
...
export class SliderLinear extends UIElement {
  ...
  private _thumbRingMesh: Mesh;
  private _thumbCoreMesh: Mesh;
  ...
  constructor(opts: SliderLinearOptions = {}) {
    ...
    this._trackW = tw;
    this._trackH = 1.6;
    this._trackRowHeight = Math.max(14, rowH);
    this._thumbSize = Math.max(8, Math.min(14, this._trackRowHeight - 4));
    ...
    this._thumbRingMesh = new Mesh(new RingGeometry(0.34, 0.5, 32));
    this._thumbRingMesh.name = "slider-thumb-ring";
    this._trackRow.add(this._thumbRingMesh);

    this._thumbCoreMesh = new Mesh(new CircleGeometry(0.2, 24));
    this._thumbCoreMesh.name = "slider-thumb-core";
    this._trackRow.add(this._thumbCoreMesh);
  ...
  applyTheme(theme: UITheme): this {
    ...
    this._trackMesh.material = theme.createFlatMaterial("line1");

    this._fillMesh.material = theme.createFlatMaterial("accentA");

    this._thumbRingMesh.material = theme.createFlatMaterial("accentA");
    this._thumbCoreMesh.material = theme.createFlatMaterial("bg0");

    this._labelText?.applyTheme(theme);
    this._readoutText?.applyTheme(theme);
    this.registerHitRegion({ slop: 3 });
  ...
  onUpdate(): void {
    ...
    this._thumbRingMesh.scale.set(this._thumbSize, this._thumbSize, 1);
    this._thumbRingMesh.position.set(tx, -midY, 0.004);

    this._thumbCoreMesh.scale.set(this._thumbSize * 0.45, this._thumbSize * 0.45, 1);
    this._thumbCoreMesh.position.set(tx, -midY, 0.005);
  }
}
